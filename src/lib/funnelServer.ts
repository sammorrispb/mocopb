...
export const SITE_ID = "mocopb" as const;
...
export async function sendFunnelEvent(params: ServerEventParams): Promise<void> {
  const secret = process.env.FUNNEL_INGEST_SECRET_MOCOPB;
  if (!secret) {
    console.warn("[funnel] FUNNEL_INGEST_SECRET_MOCOPB not set — skipping");
    return;
  }

  const email = params.email ? params.email.trim().toLowerCase() : "";
  const visitorId = params.visitorId ?? "";
  const marketingRef = params.marketingRef ?? "mocopb";
